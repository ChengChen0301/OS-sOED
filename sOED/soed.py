...
class SOED(object):
    ...
    def __init__(self, model_fun, 
        ...
        self.ds_hist = None
        self.xps_hist = None
        self.ys_mc_hist = None
        self.immediate_rewards_hist = None
        self.stop_rewards_hist = None
        self.part_continue_rewards_hist = None
        self.part_stop_rewards_hist = None
        self.averaged_continue_reward_hist = None
        self.averaged_stop_reward_hist = None
        self.averaged_stopping_stage_hist = None
        self.averaged_total_reward_hist = None
        self.indexes = None
        ...
        # self.stopping_optimizer = None
        # self.stopping_lr_scheduler = None
        ...
        # Store prior_info as instance variable
        self.prior_info = prior_info.copy() if isinstance(prior_info, np.ndarray) else list(prior_info)
        ...
        self.init_xb = self.get_xb(None)  # get belief from the prior
    ...
    def get_exact_kld_reward(self, stage, prior_mean, prior_std, post_mean, post_std):
        """
        Compute exact KL divergence between prior and posterior Gaussian distributions.
        ...
        """
        # KL(post||prior) = log(σ₀/σ₁) + (σ₁² + (μ₁-μ₀)²)/(2σ₀²) - 1/2
        kl_div = np.sum(
            np.log(prior_std/post_std) + 
            (post_std**2 + (post_mean - prior_mean)**2)/(2*prior_std**2) - 
            0.5
        )
        return kl_div

    def update_gaussian_params(self, old_mean, old_std, obs, noise_std, param_idx):
        """
        Update Gaussian parameters for the selected parameter using Bayes rule.
        
        Parameters
        ----------
        old_mean : np.ndarray
            Current mean estimates for each parameter
        old_std : np.ndarray
            Current standard deviation estimates for each parameter
        obs : float
            Observed value
        noise_std : float
        ...
        -------
        tuple
            (new_mean, new_std) for all parameters
        """
        new_mean = old_mean.copy()
        new_std = old_std.copy()
        
        # Only update the selected parameter
        precision = 1/(old_std[param_idx]**2)
        noise_precision = 1/(noise_std**2)
        
        post_precision = precision + noise_precision
        post_std = 1/np.sqrt(post_precision)
        
        post_mean = (old_mean[param_idx] * precision + obs * noise_precision)/post_precision
        
        new_mean[param_idx] = post_mean
        new_std[param_idx] = post_std
        
        return new_mean, new_std

    def compute_accumulated_kld(self, ds_hist, ys_hist):
        """
        Compute accumulated KL divergence recursively from design and observation history.
        
        Parameters
        ----------
        ds_hist : np.ndarray
            History of designs, shape (n_steps, n_design) or (n_steps,) for categorical
        ys_hist : np.ndarray
            History of observations, shape (n_steps, n_obs)
            
        Returns
        -------
        tuple
            (accumulated_kld, mean_history, std_history)
            - accumulated_kld: KL divergence at each step
            - mean_history: Mean estimates after each step
            - std_history: Standard deviation estimates after each step
        """
        n_steps = len(ys_hist)
        
        # Initialize tracking arrays
        accumulated_kld = np.zeros(n_steps)
        mean_history = np.zeros((n_steps + 1, self.n_param))
        std_history = np.zeros((n_steps + 1, self.n_param))
        
        # Start with prior
        mean_history[0] = np.array([info[1] for info in self.prior_info])
        std_history[0] = np.array([info[2] for info in self.prior_info])
        
        # Recursively update and compute KL divergence
        for t in range(n_steps):
            # Get parameter index from design
            if ds_hist.shape[1] if len(ds_hist.shape) > 1 else 1 == 1:
                # Categorical case - design is the parameter index
                param_idx = int(ds_hist[t])
            else:
                # Continuous case - design specifies parameter
                param_idx = t % self.n_param  # Cycle through parameters
                
            # Update parameters
            next_mean, next_std = self.update_gaussian_params(
                mean_history[t],
                std_history[t],
                ys_hist[t],
                self.noise_b_s,
            ...
            mean_history[t + 1] = next_mean
            std_history[t + 1] = next_std
            
            # Compute KL divergence from prior to current
            accumulated_kld[t] = self.get_exact_kld_reward(
                t,
                mean_history[0], std_history[0],  # Prior
                next_mean, next_std  # Current posterior
            )
            
        return accumulated_kld, mean_history, std_history
