...
class PGsOED(SOED):
    ...
    def soed(self, n_update=100, n_traj=1000,
        ...
        for l in range(n_update):
            ...
            for k in range(self.n_stage):
                ...
                if k < fixed_stopping_stage:  # force to continue
            ...
            stop_rewards = self.stop_rewards_hist[indexes, fixed_stopping_stage]
    ...
    def asses(self, n_traj=10000, design_noise_scale=None,
        ...
        nsamples = 50
        ys_mc_hist = np.zeros((nsamples, self.n_stage, n_traj))
        ...
        for k in range(self.n_stage + 1):
            if k < self.n_stage:
                ...
                ys_mc = np.random.normal((Gs + self.noise_loc).reshape(1, -1),
                                         (self.noise_b_s
                                      + self.noise_r_s * np.abs(Gs)).reshape(1, -1), (nsamples, n_traj))
